...
my_waffle2 <- function(x, rows = 5, use_glyph = 'square', glyph_size = 6,
                  mycolor='red', bottomMarg=-10) {
  
  len <- sum(x)
  waffles <- seq(len) - 1
  nms <- if(is.null(names(x))) seq_along(x) else names(x)
  df <- data.frame(yvals = waffles %/% rows,
                   xvals = 1 - (waffles %% rows),
                   fill = factor(rep(nms, times = x)))
  ...
  ggplot(df, aes(xvals, yvals)) +
    ...
    labs(color = NULL) +
    theme(
          plot.margin = margin(bottomMarg, 0, 0, 0),
          # plot.title = element_text(hjust = 0.5,vjust=myVJ, margin=margin(r=800))
          # plot.text(size = 18, face = 'bold',margin=margin(l=20)),
          # plot.subtitle = element_text(size = 12, color  ='grey30', margin=margin(t=10,l=20)),
          # plot.caption = element_text(size = 8, margin = margin(t=15)),
          )
} 
...
my_waffle(dubois_week10$Number,
  ...
  facet_wrap(~dubois_week10$labels)+
...
### FACET WRAPPED WAFFLE ----
teachers <- dubois_week10 |> 
  filter(Occupation=="TEACHERS")

housewives <- dubois_week10 |> 
  filter(Occupation=="HOUSE WIVES")

other <- dubois_week10 |> 
  filter(Occupation=="OTHER PROFESSIONS")

ministers <- dubois_week10 |> 
  filter(Occupation=="MINISTERS")

gov <- dubois_week10 |> 
  filter(Occupation=="GOVERNMENT SERVICE")

biz <- dubois_week10 |> 
  filter(Occupation=="BUSINESS")

iron(my_waffle2(teachers$Number,
          rows=30, 
          use_glyph ="child", 
          glyph_size=8,
          mycolor=pan_colors2[1]),
     my_waffle2(housewives$Number,
               rows=30, 
               use_glyph ="child", 
               glyph_size=8,
               mycolor=pan_colors2[2],
               bottomMarg  =-20),
     my_waffle2(ministers$Number,
                rows=30, 
                use_glyph ="child", 
                glyph_size=8,
                mycolor=pan_colors2[3],
                bottomMarg  =-80),
     my_waffle2(other$Number,
                rows=30, 
                use_glyph ="child", 
                glyph_size=8,
                mycolor=pan_colors2[4],
                bottomMarg  =-140),
     my_waffle2(gov$Number,
                rows=30, 
                use_glyph ="child", 
                glyph_size=8,
                mycolor=pan_colors2[5],
                bottomMarg  =-140),
     my_waffle2(biz$Number,
                rows=30, 
                use_glyph ="child", 
                glyph_size=8,
                mycolor=pan_colors2[6],
                bottomMarg  =-140)
     )
