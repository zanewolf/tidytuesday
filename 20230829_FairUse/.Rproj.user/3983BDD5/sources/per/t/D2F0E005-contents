...
## beeswarm plot
data <- read_csv("fair_use_cases.csv")

#need to define new categories, separating categories by first ; into "main categories"
df <- data %>% 
  mutate(categoriesOG = categories) %>% 
  separate(categories, c("categories_full", "main"), ";",extra="drop", fill='right')
df2 <- df %>% 
  separate(categories_full, c("categories_main", "categories_extra"), ",",extra="drop", fill='right') 


str_replace(df$categories_full,"Education/Research/Scholarship","Education/Scholarship/Research")
unique(df$categories_full)

# need to clean up further. Separate out "circuit" from strings



# library(beeswarm)
beeswarm(df$fair_use_found~df$categories_main)
# 
ggplot(df, aes(categories_main, fair_use_found,)) +
  geom_point() +
  geom_jitter(aes(color=fair_use_found))
...
ggplot(plotData, aes(x =df2.categories_main , y = df2.fair_use_found))+
  geom_dotplot(
    binaxis = "y",
    stackdir = "center"
  )
